...
\begin{frame}{Motivation: Stabilisation Policy in Multi-Sector Economies}
  \textbf{Starting point.} In one-sector New Keynesian models, monetary policy achieves the first-best (Aoki, 2001).  With multiple sectors, this breaks down: sectoral heterogeneity creates trade-offs.
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}
        ...
        \item Monetary policy alone is constrained-optimal
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}
        \setlength{\itemsep}{5pt}
        \item Sectoral fiscal instruments can target individual sectors (Cox et al.,~2024)
        \item With enough instruments, efficiency is restorable \textit{in theory} (La'O \& Tahbaz-Salehi,~2025; Antonova \& M\"{u}ller,~2025)
        \item But: how many instruments? What frictions matter?
      \end{itemize}
    \end{column}
  \end{columns}
...
% ============================================================
% SLIDE 2 --- RUBBO + COX (TECHNICAL DETAIL)
% ============================================================

\begin{frame}{IO Economies: Divine Coincidence Fails, Fiscal Policy Helps}
  {\small Two key results establish the tension between these literatures.}
  ...
  \medskip
  \textbf{The open question:} Rubbo shows IO breaks divine coincidence.  Cox et al.\ show fiscal policy approximately restores it, but assume \red{no IO linkages}.  What happens when we combine both features?
\end{frame}
...
\begin{frame}{The Theoretical Benchmark: $2N$ Instruments Restore Efficiency}
  ...
  \medskip
  \textbf{La'O \& Tahbaz-Salehi (2025) and Antonova \& M\"{u}ller (2025):}
  \smallskip
  \begin{itemize}
    \setlength{\itemsep}{4pt}
    \item \textbf{Result:} $2N$ sector-specific taxes (sales + production) implement the \textbf{efficient allocation} (Ramsey optimum / flexible-price outcome).
    \item \textbf{Mechanism:} Jointly stabilizing seller prices and generating efficient relative-price movements for buyers.
  \end{itemize}
...
\begin{frame}{The Gap: From Benchmark to a more realistic Economy}
  ...
  \medskip
  \textbf{The friction interaction:} L\&TS's $2N$ instruments are all \textbf{price-side} taxes.  With flexible wages, the wage adjusts freely to absorb labour misallocation.  With \red{Calvo wage setting}, the wage is a state variable: the labour tax $\tau^w_k$ \textit{interacts} with the wage friction, creating a key distortion that price-side instruments alone cannot address.
\end{frame}
