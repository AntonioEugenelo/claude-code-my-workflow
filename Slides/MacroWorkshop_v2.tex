...
\begin{frame}{Motivation: Sectoral Heterogeneity and Stabilisation Policy}
  ...
  \medskip
  \medskip
  \textbf{Research Question:} Can restricted fiscal instruments available in open economies achieve the efficiency gains predicted by theoretical benchmarks, or does the interaction between production networks and wage rigidity require a fundamentally different approach to spending?
\end{frame}
...
\begin{frame}{IO Economies: Divine Coincidence Fails, Fiscal Policy Helps}
  ...
  \begin{alertblock}{The open question}
    Rubbo shows IO breaks divine coincidence.  Cox et al.\ (``Optimal FP'') show fiscal policy restores it; however, they assume \red{no IO linkages}.  Both models rely on multisectoral heterogeneity.  What happens when we combine both features?
  \end{alertblock}
\end{frame}
...
\begin{frame}[label=production-network]{Fiscal Instruments in the Production Network}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \]
      \endgroup
      {\scriptsize (shock terms $u^w$, $u^p$ omitted; see \hyperlink{tariff-propagation}{appendix})}
    \end{column}
    \begin{column}{0.47\textwidth}
      \centering
      \begin{tikzpicture}[
...
\begin{frame}{From Subsidies to Government Spending}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Departure from Rubbo (2023)}
        \begin{itemize}
...
\begin{frame}{Research Agenda: Fiscal Policy as Demand Composition}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Revenue-neutral fiscal policy:} sectoral spending $G_{ki}$ funded by labour taxes $\tau^w_k$. The government reallocates demand across the network while wage-setters face the tax wedge.

    \item \textbf{Two policy regimes:} endogenous level + composition vs.\ fixed aggregate budget (composition only).  Quantify the welfare gap relative to the Phase~1 wedge benchmark and the L\&TS $2N$ ceiling.

    \item \textbf{Network amplification:} how does IO structure shape the optimal sectoral allocation of $G$?  Upstream vs.\ downstream spending; interaction with heterogeneous price stickiness.
  \end{enumerate}

  \medskip
  {\small\textbf{Conjecture:} sectoral \textit{composition} of spending matters more than its aggregate level. The network propagates sectoral $G$ shocks asymmetrically.}
\end{frame}
...
\begin{frame}{Summary}
  \begin{enumerate}
    ...
    \item \textbf{The Research Agenda:} Moving from subsidies to government spending $G$ funded by labour taxes.  Cox et al.\ (``Big~G'') show procurement is concentrated and compositionally distinct.

    \item \textbf{Conjecture:} Sectoral \textit{composition} of spending matters more than its aggregate level. The network propagates sectoral $G$ shocks asymmetrically.
  \end{enumerate}
\end{frame}
