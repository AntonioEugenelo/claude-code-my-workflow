...
\begin{frame}{IO Economies: Divine Coincidence Fails}
  ...
  \bigskip
  In practice, wages are sticky and instruments are restricted. What can limited fiscal tools achieve?
\end{frame}
...
\begin{frame}{The Gap: From Benchmark to a More Realistic Economy}
  ...
  \bigskip
  With \red{Calvo wages}, the wage is a state variable: price-side instruments cannot absorb wage misallocation. 
\end{frame}
...
\begin{frame}[label=production-network]{Fiscal Instruments in the Production Network}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \textbf{Price PC:} production subsidy $\tau^s_{k}$:
      \begingroup\small
      \[
        \pi_{ki,t} = \kappa_{ki}\!\left(\widehat{\mathrm{mc}}_{ki,t} {-} \hat{p}_{ki,t} \;\red{{-}\; \hat{\tau}^s_{k,t}}\right) + \beta\mathbb{E}_t\pi_{ki,t+1}
      \]
      \endgroup
    \end{column}
    \begin{column}{0.47\textwidth}
      ...
      \smallskip
      {\scriptsize $i,j,m$: sectors in country $k$ \\ 
      $\omega_{ij}$: IO expenditure share ($i$ sources from $j$)\\}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Research Agenda}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Network amplification:} Upstream vs.\ downstream spending.

    \item \textbf{Two policy regimes:} endogenous level and composition of government procurements vs.\ fixed aggregate budget (composition only).
...
\begin{frame}{Summary}
  ...
  \begin{tabular}{l c ccc c}
    ...
    Aguilar et al.\ + ext.        & $\checkmark$ & & $\checkmark$ &              & Rigid    \\
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Why government spending?}
        \begin{itemize}
          ...
          \item Subsidies ($\tau^w_k$, $\tau^s_{k}$) enter as cost-push wedges in Phillips curves
