...
**Size limit:** Keep under 200 lines (stays in the agent's context window)
...
- Builds `.agent/state/personal-memory.md` specific to desktop setup

**Machine B (laptop):**
- Clone same repo → gets same MEMORY.md ✓
- Gets same infrastructure ✓
- Builds DIFFERENT `.agent/state/personal-memory.md` for laptop setup
...
Configure in GEMINI.md for your setup.
```
...
```markdown
Configure bibliography location in GEMINI.md:
...
### GEMINI.md Can Have Placeholders

The template GEMINI.md has `[YOUR PROJECT NAME]`, `[YOUR INSTITUTION]` — this is correct. Users fill them in.
...
- Cross-machine workflows change (e.g., the IDE adds cloud sync)
...
| Machine-specific setup | ❌ No | .agent/state/personal-memory.md | ❌ No (gitignored) |
| Templates (generic) | ✅ Yes | templates/ | ✅ Yes |
| Skills (generic) | ✅ Yes | .agent/skills/ | ✅ Yes |
| Rules (path-scoped, generic) | ✅ Yes | .agent/rules/ | ✅ Yes |
| Workflows (generic) | ✅ Yes | .agent/workflows/ | ✅ Yes |
| Hook-like rules (behavior) | ✅ Yes | .agent/rules/ | ✅ Yes |
| Session logs | ✅ Yes | quality_reports/session_logs/ | ✅ Yes |
| Plans | ✅ Yes | quality_reports/plans/ | ✅ Yes |
| Local settings | ❌ No | (managed by IDE) | ❌ No |
| Session state | ❌ No | .agent/state/ | ❌ No (gitignored) |
...
- Keep specific learnings local (.agent/state/personal-memory.md, gitignored)
