...
def format_learn_reminder(percentage: float, threshold: int) -> str:
    ...
    return f"""
{CYAN}[context] Estimated context at {percentage:.0f}%{NC}

Non-obvious discovery or reusable workflow?
-> Consider capturing it as a skill before context compacts.

Codex skills are saved to {MAGENTA}.codex/skills/{NC} and persist in this workflow.
"""
...
def format_warn_80(percentage: float) -> str:
    ...
    return f"""
{YELLOW}[context] Estimated context at {percentage:.0f}%{NC}

Auto-compact will handle context management automatically.
Be aware that context may be summarized soon.
"""
...
def format_warn_90(percentage: float) -> str:
    ...
    return f"""
{RED}[context] Estimated context at {percentage:.0f}% - auto-compact approaching{NC}

Complete current task with full quality. Do NOT cut corners or skip verification.
Auto-compact should preserve important information, but write down key state now.
